  ...
  return (
    <div className="space-y-6 animate-fade-in p-6">
      <header className="mb-8 border-b border-slate-800 pb-4">
        <h2 className="text-2xl font-bold text-white tracking-tight">Knowledge Inventory</h2>
        <p className="text-slate-400">Analysis of {documents.length} ingested documents.</p>
      </header>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Detected Conflicts & Anomalies</h3>
        {allConflicts.length === 0 ? (
            <div className="text-slate-500 italic">No internal conflicts detected across documents.</div>
        ) : (
            <div className="space-y-3">
                {allConflicts.map((c, i) => (
                    <div key={i} className="flex items-start gap-3 bg-red-950/20 border border-red-900/50 p-4 rounded-lg">
                        <div className="text-red-500 mt-1">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                        </div>
                        <div>
                            <div className="font-mono text-red-200 text-sm">{c.parameter}</div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Parameter Registry</h3>
                ...
                <tbody className="text-sm">
                    {allEntities.slice(0, 10).map((e, i) => (
                        <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/50">
                            <td className="py-2 px-3 font-mono text-blue-300">{e.name}</td>
                            <td className="py-2 px-3 text-slate-200">{e.value}</td>
                            <td className="py-2 px-3 text-slate-400">{e.unit || '-'}</td>
                            <td className="py-2 px-3"><span className="text-xs bg-slate-800 px-2 py-1 rounded border border-slate-700">{e.type}</span></td>
                            <td className="py-2 px-3 text-slate-500 truncate max-w-[150px]">{documents.find(d => d.id === e.sourceId)?.filename}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
            {allEntities.length > 10 && (
                <div className="text-center mt-4 text-xs text-slate-500">Showing 10 of {allEntities.length} entities</div>
            )}
        </div>
      </div>
    </div>
...
const StatCard = ({ title, value, alert = false }: { title: string, value: number, alert?: boolean }) => (
  <div className={`p-5 rounded-xl border ${alert ? 'bg-red-950/10 border-red-900/50' : 'bg-slate-900 border-slate-800'}`}>
    <div className="text-slate-400 text-sm font-medium uppercase tracking-wider">{title}</div>
    <div className={`text-3xl font-bold mt-2 font-mono ${alert ? 'text-red-400' : 'text-white'}`}>{value}</div>
  </div>
);
